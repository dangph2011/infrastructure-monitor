<ul class="sidebar-menu" data-widget="tree">
    <li class="header">MAIN NAVIGATION</li>
    <li class="active"><a href="#"><i class="fa fa-link"></i><span>Topo</span></a></li>

    <li class="treeview">
        <a href="#"><i class="fa fa-pie-chart"></i> <span>Graphs</span>
                <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
                </span>
                </a>
        <ul class="treeview-menu">
            {{-- <li><a href="/graph/create">Create graph</a></li> --}}
            <li><a href="/graph">View graph</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-exclamation-circle"></i> <span>Problems</span>
                <span class="pull-right-container">
                <i class="fa fa-angle-left pull-right"></i>
                </span>
                </a>
        <ul class="treeview-menu">
            <li><a href="/problems/trigger/create">Create trigger</a></li>
            <li><a href="/problems/problem">Problem</a></li>
            <li><a href="/problems/trigger">Trigger</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-sticky-note-o"></i> <span>Reports</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/report">View report</a></li>
            <li><a href="/report/create">Create report</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-plug"></i> <span>Replication</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/replication">View</a></li>
            <li><a href="/replication/create">Create</a></li>
            <li><a href="/replication/history">History</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-plug"></i> <span>Dump</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/dump">View</a></li>
            <li><a href="/dump/create">Create</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-plug"></i> <span>Local Server</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/local">View</a></li>
            <li><a href="/local/create">Create</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-plug"></i> <span>Manage User</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/users">View</a></li>
            <li><a href="/users/create">Create</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-plug"></i> <span>Manage Role</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/roles">View</a></li>
            <li><a href="/roles/create">Create</a></li>
        </ul>
    </li>

    <li class="treeview">
        <a href="#"><i class="fa fa-plug"></i> <span>Manage Permission</span>
                    <span class="pull-right-container">
                    <i class="fa fa-angle-left pull-right"></i>
                    </span>
                    </a>
        <ul class="treeview-menu">
            <li><a href="/permissions">View</a></li>
            <li><a href="/permissions/create">Create</a></li>
        </ul>
    </li>


    {{--
    <li><a href="#"><i class="fa fa-link"></i> <span>Another Link</span></a></li> --}} {{--
    <li class="treeview">
        <a href="#"><i class="fa fa-link"></i> <span>Multilevel</span>
            <span class="pull-right-container">
            <i class="fa fa-angle-left pull-right"></i>
            </span>
            </a>
        <ul class="treeview-menu">
            <li><a href="#">Link in level 2</a></li>
            <li><a href="#">Link in level 2</a></li>
        </ul>
    </li> --}}
</ul>
